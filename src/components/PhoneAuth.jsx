...
export default function PhoneAuth() {
  ...
  // Clean up the recaptcha widget whenever this component unmounts (e.g.
  // navigating from Register to Login) — otherwise window.recaptchaVerifier
  // keeps pointing at a DOM node that no longer exists, and the next
  // attempt fails with "reCAPTCHA client element has been removed".
  useEffect(() => {
    return () => {
      clearRecaptcha();
    };
  ...
  const setupRecaptcha = () => {
    // Always start from a clean slate so a stale verifier from a previous
    // mount/attempt never gets reused against a removed DOM node.
    clearRecaptcha();
    window.recaptchaVerifier = new RecaptchaVerifier(auth, "recaptcha-container", {
      size: "invisible",
    });
